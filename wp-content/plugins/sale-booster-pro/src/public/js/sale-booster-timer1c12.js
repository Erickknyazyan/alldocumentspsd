    ...
    configs.forEach(config => {
        var boostApp = {
            ...
            getUserLandedTimeSpan: function() {
                var cookieParam = '_boost_product_'+config.product_id;
            ...
            initClock: function($wrapper, timeLeft) {
                if (timeLeft < 0) {
                    // hide pin template
                    $('._salebooster_pin_template_'+config.product_id).hide();
                ...
                var clockInnerHtml = "<div class='_sale-discount-countdown-timer'>" +
                    ...
                    "   <span class='_sale-discount-units'>"+config.trans.days+"</span>" +
                    "</div>" +
                    "<div class='_sale-discount-countdown-timer'>" +
                    "  <span class='_sale-discount-time _timer_hours'></span>" +
                    "  <span class='_sale-discount-units'>"+config.trans.hours+"</span>" +
                    "</div>" +
                    "<div class='_sale-discount-countdown-timer'>" +
                    "   <span class='_sale-discount-time _timer_minutes'></span>" +
                    "   <span class='_sale-discount-units'>"+config.trans.minutes+"</span>" +
                    "</div>" +
                    "<div class='_sale-discount-countdown-timer'>" +
                    "   <span class='_sale-discount-time _timer_seconds'></span>" +
                    "   <span class='_sale-discount-units'>"+config.trans.seconds+"</span>" +
                    "</div>"
                ...
                function updateTime() {
                    ...
                    $('._salebooster_pin_template_'+config.product_id).show();
            ...
            initTimers: function() {
                var clocks = $('._sale-booster-countdown_'+config.product_id);
                var timestamp = config.timerSeconds;
                // now we have to calculate the timestamp here
               if (config.type == 'user_based_time') {
